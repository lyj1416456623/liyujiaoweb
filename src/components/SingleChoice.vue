<template>
  <div class="single-choice">
    <div class="single-title">
      <template v-for="item in singleData.title">
    ...
    <div class="qusition-option" v-for="(val,key,i) in singleData.optionMap" :key="'op'+key"
         :class="checkedValue == key?'single-active':''">
      <label :for="'single'+key+i" class="question-block">
      ...
      <input type="radio" name="single" :id="'single'+key+i" :value="key" v-model="checkedValue"/>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    watch: {
      checkedValue(val) {
        ...
        this.singleData.studentAnswer = val;
        this.$emit("single-check", val);
      }
    },
    mounted() {
      console.log('single mounted hsq1111111', this.singleData)
      if(this.answer && this.answer.length>0){
        this.checkedValue = this.answer[0];
      }
    },
    methods: {}
  }
